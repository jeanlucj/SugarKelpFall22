...
mrkCallRate <- apply(dartagMrk[, 17:204], 1, function(v) sum(!is.na(v))/188)
jpeg("./output/HistMrkCallRate.jpeg")
hist(mrkCallRate)
dev.off()

indCallRate <- apply(dartagMrk[, 17:204], 2, function(v) sum(!is.na(v))/nrow(dartagMrk))
...
mrkFreqHet <- apply(dartagMrk[, 17:204], 1, function(v) sum(v == 2, na.rm=T) / sum(!is.na(v)))
hist(mrkFreqHet)
plot(dartagMrk$AvgPIC, mrkFreqHet)

indFreqHet <- apply(dartagMrk[, 17:204], 2, function(v) sum(v == 2, na.rm=T) / sum(!is.na(v)))
